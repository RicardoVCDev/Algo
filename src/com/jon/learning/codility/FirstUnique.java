...
public class FirstUnique {
    public static void main(String[] args) {
        ...
        System.out.println("Ans: "+ new FirstUnique().solution(arr));
    }
    ...
    public int solution(int[] A) {
        ...
        for (int i = 0; i < A.length; i++) {
            if (nums.get(A[i]) == null) {
                nums.put(A[i], i);
                unique.put(i, A[i]);
                // System.out.println("Added: {"+i+","+A[i]);
            } else {
                unique.put(nums.get(A[i]), -1);
                // System.out.println("Updated: {"+nums.get(A[i])+",-1");
            }
        }
